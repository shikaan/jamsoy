...
class CPU {
  ...
  static MAX_CYCLES = CPU.CLOCK_SPEED / 60; // CLOCK_SPEED / 60 frames per second
  ...
  #execute(debug = false) {
    ...
    switch (opcode) {
      ...
      case 0x22:
        // LD (HL+), A
        this.#memory.writeByte(this.#register.HL, this.#register.A & 0xFF);
        this.#register.HL++;
        return 8;
      case 0x23:
        // INC HL
      ...
      case 0x76:
        // HALT
        const haltBug = this.#interrupts.hasPendingInterrupts() && !this.#interrupts.IME;
      ...
      case 0xD9:
        // RETI
        this.#register.PC = this.#pop();
        this.#interrupts.enableInterrupts(true, 0);
        return 16;
      case 0xDA:
        // JP C, a16
        if (this.#register.flagCarry) {
   ...
   */
  #inc8(r8) {
    const initial = this.#register[r8];
    this.#register[r8]++;
    this.#register.flagZero = this.#register[r8] == 0
    this.#register.flagSubtract = false
    this.#register.flagHalfCarry = isHalfCarry8(initial, 1)
  }
  ...
  #inc8ref(ref) {
    ...
    this.#register.flagZero = this.#memory.readByte(ref) === 0;
    this.#register.flagSubtract = false;
    this.#register.flagHalfCarry = isHalfCarry8(initial, 1);
  }
  ...
  #dec8(r8) {
    ...
    this.#register.flagZero = this.#register[r8] === 0
    this.#register.flagSubtract = true
    this.#register.flagHalfCarry = isHalfBorrow8(initial, 1)
  }
  ...
  #rst(val) {
    this.#register.SP -= 2
    this.#memory.writeWord(this.#register.SP, this.#register.PC)
    this.#register.PC = val
  }
