...
const shiftLeft8 = (/** @type {number} */ x) => [x << 1, x >> 7]
const shiftRight8 = (/** @type {number} */ x) => [x >> 1, x & 1]
...
class CPU {
  ...
  #getOpcode() {
    // if ((this.#register.PC >= 0x4000 && this.#register.PC <= 0x7FFF) || (this.#register.PC >= 0xA000 && this.#register.PC <= 0xBFFF))
    return this.#memory.readByte(this.#register.PC)

    // return this.#memory.unsafeReadROMByte(this.#register.PC);
  }
  ...
  #execute(debug = false) {
    ...
    if (debug) {
      ...
      console.log(`${format.hex(this.#register.PC - 1)} ${format.instruction(instruction)} ;;${format.hex(opcode)}`);
    }
  ...
  #executeCB(debug = false) {
    ...
    if (debug) {
      ...
      console.log(`${format.hex(this.#register.PC - 1)} ${format.instruction(instruction)}`);
    }
   ...
   */
  #inc8ref(ref) {
    const value = (this.#memory.readByte(ref) + 1) & 0xFF;
    this.#register.flagZero = value === 0;
    this.#register.flagSubtract = false;
