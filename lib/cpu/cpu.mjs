...
class CPU {
   ...
   */
  constructor(memory, register = r, decode = d) {
 ...
 */
  ...
  run(code) {
    ...
    const cycles = this.#execute(instruction, this.#register, this.#memory);
    setTimeout(() => this.run(code), cycles * CYCLE_TIME_MS);
  }
}
