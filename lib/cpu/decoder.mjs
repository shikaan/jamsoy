...
Object.entries(opcodes.cbprefixed).forEach(
  ([k, v]) => (PREFIXED[Number.parseInt(k, 16)] = v),
);

const UNPREFIXED = new Array(256);
Object.entries(opcodes.unprefixed).forEach(
  ([k, v]) => (UNPREFIXED[Number.parseInt(k, 16)] = v),
);
...
function decode(address, data) {
  if (address >= data.length) throw new Error(`Invalid address ${address} with data length ${data.length}`);
