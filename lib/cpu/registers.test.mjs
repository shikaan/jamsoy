  ...
  describe("word registers", () => {
    ...
    for (const r of WORD_REGS) {
      describe(r, () => {
        it("gets and sets correct values", () => {
          register[r] = 0xabcd;
          assert.deepEqual(register[r], 0xabcd, `${r} does not match`);
          if (WORD_TO_BYTES[r].length > 0) {
            assert.deepEqual(register[WORD_TO_BYTES[r][0]], 0xab, `${r} lower nibble does not match`);
            assert.deepEqual(register[WORD_TO_BYTES[r][1]], 0xcd, `${r} upper nibble does not match`);
          }
        });
    ...
    describe("AF", () => {
      it("gets and sets correct values", () => {
        register.AF = 0xab10;
        assert.deepEqual(register.AF, 0xab10, `0x${register.AF.toString(16)} does not match`);
        assert.deepEqual(register.A, 0xab);
        assert.deepEqual(register.F, 0x10);
      });

      it("gets and sets out of bounds values", () => {
        register.AF = 0xfffff;
        assert.deepEqual(register.AF, 0xfff0);
        assert.deepEqual(register.A, 0xff);
        assert.deepEqual(register.F, 0xf0);
      });
    })
  });
