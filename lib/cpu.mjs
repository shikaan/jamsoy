...
import { add, adc, sub, sbc, or, and, xor, cp, inc, ld, ldh, push, pop, daa, cpl, swap, ccf, scf, rlc, rl, rrc, rr, sla, sra, srl, bit, set, res, dec, jp, jr, call, rst, ret } from "./instructions/index.mjs";
...
function execute(intruction, register, memory) {
  switch (intruction.mnemonic) {
    ...
    case "CALL": return call(intruction, register, memory)
    case "RST": return rst(intruction, register, memory)
    case "RET": return ret(intruction, register, memory)
    case "RETI": {
      allowInterrupts = true;
...
function run(code, decode = d, register = r, memory = m) {
  while (true) {
    ...
    console.log(`executing: ${format.hex(address)} ${format.instruction(instruction)}`)
