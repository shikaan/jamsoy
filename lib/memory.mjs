...
class Memory {
  ...
  writeByte(address, data) {
    ...
    if (within(address, 0, 0x7FFF)) {
      ...
      if (address === 0xFF00) {
        ...
        // Divider
        this.#local.setUint8(address, 0);
      } else if (within(address, 0xFF05, 0xFF07)) {
        // Timer
        this.#local.setUint8(address, data);
      } else if (address === 0xFF0F) {
        // Interrupts
  ...
  */
  ...
  initialize() {
    this.#local = new DataView((new Uint8Array(0x10000)).buffer);
    this.#cartridge = new DataView((new Uint8Array(0x200000)).buffer);
    this.#writeCallbacks = new Map();
    this.#serialCallback = undefined;
