...
class Graphics {
  ...
  #drawBackgroundAndWindow(lcdc) {
    ...
    const useSignedRange = !readBit(lcdc, 4);
    const backgroundTileDataStart = useSignedRange ? 0x8800 : 0x8000
    ...
    for (let lx = 0; lx < 160; lx++) {
      ...
      const tileIndex = this.#memory.readByte(backgroundTileMapStart + tileMapCol + tileMapRow * 32, useSignedRange);

      const tileDataRow = useSignedRange ? (tileIndex + 128) : tileIndex
  ...
  #getColor(colorNum) {
    ...
    switch (color) {
      case 0: return [250, 251, 246];
      case 1: return [198, 183, 190];
      case 2: return [86, 90, 117];
      case 3: return [15, 15, 27];
      default: return [15, 15, 27];
    }
  }
}
