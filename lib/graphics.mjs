...
class Graphics {
  ...
  #drawBackground() {
    ...
    const isLineInView = windowY <= currentLine;
    ...
    const isSigned = tileBaseAddress === 0x8800;
    ...
    for (let pixelX = 0; pixelX < 160; pixelX++) {
      ...
      const tileNum = this.#memory.readByte(tileMapBaseAddress + tileRow + tileCol, isSigned);
      const tileAddress = isSigned
      ...
      const colorBit = 7 - (tileX % 8); // FIXME: eh?!?
      const colorNum = +readBit(data2, colorBit) << 1 | +readBit(data1, colorBit);

      const [r, g, b] = this.#getColor(colorNum);

      // TODO: Do we neeed to fetch the line again?
      const pixelY = this.#memory.readByte(SCANLINE_REGISTER);
