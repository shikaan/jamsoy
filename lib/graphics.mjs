...
class Graphics {
  ...
  #drawBackgroundAndWindow(lcdc) {
    ...
    const [bgX, bgY] = [this.#memory.readByte(0xFF43), this.#memory.readByte(0xFF42)];

    // Window Map
    const wTileMapRangeStart = readBit(lcdc, 6) ? 0x9C00 : 0x9800
    const [wX, wY] = [this.#memory.readByte(0xFF4B) - 7, this.#memory.readByte(0xFF4A)];
    ...
    for (let lx = 0; lx < 160; lx++) {
      ...
      const [tileMapX, tileMapY] = isDrawingWindow
        ? [(lx - wX) % 256, (ly - wY) % 256]
        : [(lx + bgX) % 256, (ly + bgY) % 256]
