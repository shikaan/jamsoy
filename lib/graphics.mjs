...
class Graphics {
  ...
  // 1 cycle == 4 dots
  // OAM -> 80 dots (20 cycles)
  // VRAM -> 172-289 dots (43-72 cycles)
  // HBLANK -> 87-204 dots (21-51 cycles)
  // VBLANK -> 4560 dots (1140 cycles)
    ...
    switch (mode) {
      case LCD_MODE.OAM:
        if (this.#cycles >= 20) {
          ...
          this.#cycles -= (this.#memory.readByte(0xFF43) % 8) / 4;
        }
        break;
      case LCD_MODE.VRAM:
        if (this.#cycles >= 43) {
      ...
      case LCD_MODE.HBLANK:
        if (this.#cycles >= 51) {
          this.#cycles -= 51;
          this.#memory.unsafeWriteByte(SCANLINE_REGISTER, this.#memory.readByte(SCANLINE_REGISTER) + 1);
  ...
  #drawSprites(lcdc) {
    ...
    for (let sprite = 0; sprite < 40; sprite++) {
      // TODO: this might have some penalty too
