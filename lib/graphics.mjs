    ...
    for (let sprite = 0; sprite < 40; sprite++) {
      ...
      if (currentLine >= y && currentLine < y + ySize) {
        ...
        for (let pixel = 7; pixel >= 0; pixel--) {
          ...
          const color = (+readBit(secondByte, colorBit) << 1) | +readBit(firstByte, colorBit);
          ...
          const palette = readBit(attributes, 4) ? SPRITE_PALETTE_REGISTER_1 : SPRITE_PALETTE_REGISTER_0;
