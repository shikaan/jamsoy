...
function ret(instruction, register, memory) {
  ...
  if (first.name === "NZ") {
    return !register.flagZero ? popAndJump(register, memory, instruction) : instruction.cycles[1];
  } else if (first.name === "Z") {
    return register.flagZero ? popAndJump(register, memory, instruction) : instruction.cycles[1];
  } else if (first.name === "NC") {
    return !register.flagCarry ? popAndJump(register, memory, instruction) : instruction.cycles[1];
