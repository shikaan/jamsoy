...
import { jp, jr, call } from './jump.mjs';
...
describe('call', () => {
  ...
  it('jumps to the specified address when operand is a16', () => {
    ...
    const cycles = call(instruction, registers, memory);
    assert.strictEqual(registers.PC, 0x1234);
  ...
  it('jumps to the specified address when operand is NZ and flagZero is false', () => {
    ...
    registers.flagZero = false;
    assert.strictEqual(call(instruction, registers, memory), 10);
    assert.strictEqual(registers.PC, 0x1234);
    assert.strictEqual(registers.SP, 0x1234);
    registers.flagZero = true;
    registers.PC = 0
    registers.SP = 0x1236;
    assert.strictEqual(call(instruction, registers, memory), 12);
    assert.strictEqual(registers.PC, 0);
    assert.strictEqual(registers.SP, 0x1236);
  });
});