...
export function rlc(instruction, register, memory) {
  ...
  const carry = value >> 7;
  setRegisterFromOperand(instruction.operands[0], register, memory, (value << 1) & 0xFF | carry);
...
export function rl(instruction, register, memory) {
  const value = getValueFromOperand(instruction.operands[0], register, memory);
  const carry = value >> 7;
  setRegisterFromOperand(instruction.operands[0], register, memory, (value << 1) & 0xFF | +register.flagCarry);
  register.flagCarry = !!carry;
  register.flagSubtract = false;
  register.flagHalfCarry = false;
  register.flagZero = value === 0;
...
export function rrc(instruction, register, memory) {
  const value = getValueFromOperand(instruction.operands[0], register, memory);
  const carry = value & 1;
  setRegisterFromOperand(instruction.operands[0], register, memory, (value >> 1) | (carry << 7));
  register.flagCarry = !!carry;
  register.flagSubtract = false;
  register.flagHalfCarry = false;
  register.flagZero = value === 0;
...
export function rr(instruction, register, memory) {
  const value = getValueFromOperand(instruction.operands[0], register, memory);
  const carry = value & 1;
  setRegisterFromOperand(instruction.operands[0], register, memory, (value >> 1) | (+register.flagCarry << 7));
  register.flagCarry = !!carry;
  register.flagSubtract = false;
  register.flagHalfCarry = false;
  register.flagZero = value === 0;
