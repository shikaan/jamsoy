...
class PPU {
  ...
  #drawSprites(lcdc) {
    ...
    const ySize = readBit(lcdc, 2) ? 16 : 8;
    ...
    for (let sprite = 0; sprite < 40; sprite++) {
      ...
      const tileDataRow = this.#memory.readByte(
      ...
      if (ly >= y && ly < y + ySize) {
        ...
        const tileDataCol = (yFlip ? ySize + y - ly - 1 : ly - y) * 2;

        // Range of the tile data is 0x8000-0x8FFF for sprites
        const lineIndex = 0x8000 + tileDataRow * 16 + tileDataCol;
        const firstByte = this.#memory.readByte(lineIndex);
        const secondByte = this.#memory.readByte(lineIndex + 1);
        ...
        for (let pixel = 7; pixel >= 0; pixel--) {
          ...
          this.#screen.setPixel(lx, ly, this.#getColor(color, palette));
        }
      }
    }
  }
