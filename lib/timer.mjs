...
class Timer {
  ...
  update(tCycles) {
    ...
    if (this.#dividerCounter >= 256) {
      ...
      this.#memory.unsafeWriteByte(DIVIDER_ADDRESS, this.#memory.unsafeReadByte(DIVIDER_ADDRESS) + 1);
    }

    if (!this.#isClockEnabled()) return;

    this.#clockCounter += tCycles * 4;
    ...
    while (this.#clockCounter >= frequency) {
      ...
      if (tima === 0xFF) {
        ...
        this.#memory.writeByte(TIMER_COUNTER_ADDRESS, this.#memory.readByte(TIMER_COUNTER_ADDRESS) + 1);
      }
    }
  }
...
export { Timer, TIMER_CONTROL_ADDRESS, TIMER_COUNTER_ADDRESS, TIMER_MODULO_ADDRESS, DIVIDER_ADDRESS };